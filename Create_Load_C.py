...
BD_PRINCIPAL = "db_cloned"                         # Substituir pelo nome do banco de dados
...
class Pessoa(Base):
    ...
    postagem =        Column(String(1000), nullable=False)
    sexo =            Column(String(1),    nullable=False)  # A coluna 'sexo' aceita apenas 1 caractere
    ...
    with sessao.begin():
        for cpf in numeros_cpf_unicos:
            novo_registro = Pessoa(
                ...
                postagem=fake.text(),
                sexo=random.choice(['M', 'F'])  # Use a função personalizada aqui
            )
